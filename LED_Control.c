...
int
setup_LED(volatile uint8_t* address, int port_num)
{
  volatile uint8_t* ddrE = (volatile uint8_t*)(0x0D + 0x20);

  listOfLED[listOfLEDIndex].ID =
    listOfLEDIndex; // set LED ID in struct using ID in array
